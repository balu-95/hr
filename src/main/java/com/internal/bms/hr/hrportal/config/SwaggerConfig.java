	...
	public OpenAPI openAPI() {
		return new OpenAPI().addSecurityItem(new SecurityRequirement().addList("Bearer Authentication"))
				...
				.info(new Info().title("My REST API").description("Some custom description of API.").version("1.0")
						.contact(new Contact().name("balamurugan sivakumar").email("hellobalamurugans@gmail.com")));
	}

	@Bean
	public GroupedOpenApi candidateApi() {
		return GroupedOpenApi.builder().group("candidate").pathsToMatch("/api/v1/candidates/**").build();
	}
